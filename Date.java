...
public class Date{
	...
	public Date(int year,int month,int day){
     //至少做基本的参数检查
        if(year<1900||year>2100){//最好的做法是抛弃异常
			System.out.println("年不符合："+year);
			return;
		}	 
		if(month<1||month>12){
			System.out.println("月不符合："+month);
            return;		
		}
		if(day<1||day>getDayOfMonth(year,month)){
			System.out.println("日不符合："+day);
			return;
		}
		//涉及name shadow
		this.year=year;
	...
	public static void main(String[] args){
		...
		Date inter=new Date(2019,12,1);
		System.out.printf("距离12月1还有%d天%n",Date.differ(inter,today));
	} 	
}
